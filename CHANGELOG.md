...
- The `mat` feature moves to **hdf5-pure 0.41** (was 0.39). No beve API change and no change to the bytes written: twenty-one payload shapes were converted under both versions and compared byte for byte — scalars, numeric and complex arrays, bools, strings and string arrays, empty typed and generic arrays, `null`, a row-major matrix, a struct, a nested object, a mixed cell array, a workspace-object root, and the deflate/`V110` chunked writes among them. All twenty-one are identical.

  Upstream's breaking changes do not reach beve. The new refusals, the `AttrValue` widening, and 0.41's change to how a variable-length string attribute reads back are all read-side; beve only writes, and the `AttrValue::VarLenAsciiArray` → `VarLenAsciiCharArray` rename is a variant beve never names — the struct-field attribute written through it keeps the same bytes. beve converts hdf5-pure errors through `Display` rather than matching on them, so the new `Error::NotAGroup`/`NotANamedDatatype` are outside its surface. The option enums it re-exports (`Compression`, `NullPolicy`, `LibVer`, ...) are unchanged, the transitive dependency set is unchanged, and the declared MSRV stays 1.96.1. The bulk of these two releases is write-gathering, free-space reuse, read-write-session correctness and dense attribute storage, which the `mat` path — a single-pass build and `finish` — does not go through; `MatV73Options::libver` still pins `LibVer::V18`, so MATLAB still loads what beve writes.

  If you depend on hdf5-pure directly as well, move to 0.41 in step with this release — beve's public API is typed by the re-exported enums, so the versions must agree.

- **A `.mat` walk failure keeps its `Error` variant.** Converting a BEVE object or array walks its members inside a `MatBuilder` closure, whose signature admits only hdf5-pure's `MatError`, so a beve error raised there had to cross into hdf5-pure and back. It crossed as `MatError::Custom(e.to_string())` and came back as `Error::MessageOwned` — the error was flattened to a string and rebuilt on a round trip that never leaves this crate, and a caller that could have matched `Error::Eof` got text instead.

  It now crosses as `MatError::Source`, which 0.40 added for exactly this, and is downcast back on the way out. A truncated payload inside a struct field reports `Error::Eof` where it reported `Error::MessageOwned("unexpected end of input")`. **The message is unchanged** — both `MatError` variants print the inner error bare — so only a caller matching on the variant sees a difference, and what it sees is the variant it wanted. Errors from hdf5-pure itself still arrive as `Error::MessageOwned`, as before.
