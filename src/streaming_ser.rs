...
/// A [`Write`] that discards every byte and counts how many it was handed.
///
/// The streaming serializer routes all output through `write_all` (see
/// [`StreamingSerializer`]'s `write_byte`, `write_size`, and `write_all` helpers),
/// so the `write_all` override below is the only path exercised when measuring.
/// `write` is implemented too so `CountingWriter` is a correct [`Write`] in its own
/// right; because it always reports the full length, the default `write_all` would
/// behave identically.
#[derive(Default)]
...
/// Compute the exact number of bytes [`to_writer_streaming`] would write for
/// `value`, without producing them.
///
...
/// # Errors
///
/// Returns the same error [`to_writer_streaming`] would for `value` — in
/// particular [`Error::Unsupported`](crate::Error::Unsupported) for an
/// unknown-length sequence (`serialize_seq(None)`) or map (`serialize_map(None)`).
/// A successful measure therefore guarantees the value is streamable: a
/// subsequent [`to_writer_streaming`] will not *reject* it. It does not guarantee
/// the write itself succeeds — an I/O error can still occur mid-body — so the
/// framing layer remains responsible for tearing down the connection on a
/// mid-body write failure.
///
/// # Cost
///
/// One traversal of the value through the encoder, writing into a counter with no
/// allocation and no byte movement (each write is an integer add). The cost
/// depends on which encoder path each leaf takes:
///
/// - A [`serialize_bytes`](serde::Serializer::serialize_bytes) body
///   (`&[u8]`, `serde_bytes::Bytes`, `ByteBuf`) or a
///   [`TypedSlice<T>`](crate::TypedSlice) body is **O(1)**: a single bulk write
///   becomes one `len += n`.
/// - A bare numeric `Vec<T>` (e.g. `Vec<u8>`, `Vec<f64>`) is **O(N)**: serde
///   drives it element-by-element, so the encoder still performs N conversions
///   and N write calls even though the counter discards the bytes.
/// - General nested structures (strings, maps, heterogeneous arrays) are
///   **O(payload)**: they must be walked to sum sizes.
///
/// For a guaranteed-O(1) measure of a large numeric body, encode it through a
/// single bulk write — a `&[u8]`/`serde_bytes` blob or a
/// [`TypedSlice<T>`](crate::TypedSlice). For a contiguous numeric slice whose
/// size is needed with no traversal at all, [`typed_slice_size`](crate::typed_slice_size)
/// is a closed-form alternative.
///
/// # Example
///
/// ```rust
...
/// ```rust
/// # use std::io::Write;
/// # fn frame<W: Write, T: serde::Serialize>(mut w: W, value: &T) -> beve::Result<()> {
/// let body_len: u64 = beve::serialized_size(value)?;
/// w.write_all(&body_len.to_le_bytes())?;          // advertise the body length
/// beve::to_writer_streaming(&mut w, value)?;       // emit exactly `body_len` bytes
/// # Ok(())
/// # }
/// ```
