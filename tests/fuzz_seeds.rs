...
//! gitignored; `fuzz/seeds/` is the curated set, and only files a human put
//! there are committed.
    ...
    for path in &paths {
        ...
        assert!(
            ...
            "fuzz seed {target}/{name} failed ({} bytes); reproduce with \
             `cargo +nightly fuzz run {target} fuzz/seeds/{target}/{name}`",
            data.len()
        );
    }
    paths.len()
