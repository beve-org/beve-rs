use beve::{to_vec_complex64, to_vec_complex64_slice, Complex, Matrix, MatrixLayout};
...
fn sample_vec_f64() -> Vec<f64> {
    vec![1.1, -2.25, 3.5]
}
fn sample_vec_u32() -> Vec<u32> {
    vec![1, 2, 1000]
}
fn sample_vec_bool() -> Vec<bool> {
    vec![true, false, true, true, false]
}
fn sample_vec_string() -> Vec<String> {
...
fn interop_matrices_to_json() {
    ...
    let bytes =
        beve::fast::to_vec_matrix_f64(beve::fast::MatrixLayoutFast::Right, &[rows, cols], &data);
    ...
    assert!(
        json.contains("\"extents\":[2,3]"),
        "json missing extents: {}",
        json
    );
    assert!(
        json.contains("\"value\":[1,2,3,4,5,6]"),
        "json missing values: {}",
        json
    );
}
...
fn interop_integer_keyed_objects() {
    ...
    let mut expect = BTreeMap::new();
    expect.insert(1, 10);
    expect.insert(1u64 << 40, 20);
    assert_eq!(got, expect);

    // Rust -> Glaze validation
    fn expect_i8() -> BTreeMap<i8, i32> {
        let mut m = BTreeMap::new();
        m.insert(-1, 10);
        m.insert(2, 20);
        m
    }
    let bytes = beve::to_vec(&expect_i8()).unwrap();
    let (code, out, err) = run_glaze(&["read", "mapi8"], Some(&bytes)).unwrap();
    assert_eq!(code, 0, "{}", String::from_utf8_lossy(&err));
    assert_eq!(String::from_utf8_lossy(&out), "OK\n");

    let mut m16 = BTreeMap::new();
    m16.insert(1u16, 10);
    m16.insert(65535u16, 20);
    let (code, out, err) =
        run_glaze(&["read", "mapu16"], Some(&beve::to_vec(&m16).unwrap())).unwrap();
    assert_eq!(code, 0, "{}", String::from_utf8_lossy(&err));
    assert_eq!(String::from_utf8_lossy(&out), "OK\n");

    let mut m64 = BTreeMap::new();
    m64.insert(1u64, 10);
    m64.insert(1u64 << 40, 20);
    let (code, out, err) =
        run_glaze(&["read", "mapu64"], Some(&beve::to_vec(&m64).unwrap())).unwrap();
    assert_eq!(code, 0, "{}", String::from_utf8_lossy(&err));
    assert_eq!(String::from_utf8_lossy(&out), "OK\n");
}
